...
import { ArtifactTypeMap, FinishReasonMap } from "../app/generation/generation_pb";
import fs from 'fs';
...
export function onGenerationComplete(response: GenerationResponse) {
  ...
  // Assuming you're working in a client-side context (browser), you can create data URLs for the images.
  const imageElements = response.imageArtifacts.map(
    (artifact: Generation.Artifact) => {
      // Convert the binary image data to a base64-encoded data URL
      const imageDataUrl = `data:image/png;base64,${Buffer.from(
        artifact.getBinary_asU8()
      ).toString("base64")}`;
  ...
  // You can return the image elements if needed, but it's optional
